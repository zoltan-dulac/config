...
function Config(dataIslandId) {
    ...
    me.isClientSide = (typeof document !== 'undefined');
    if (me.isClientSide) {
    ...
    me.getScriptedValue = function (s, macros, defaultValue) {
        ...
        var r = me.isClientSide ? me.getNormalizedValue(s, defaultValue) : me.getValue(s, defaultValue);
    ...
    me.nodeInit = function (file) {
        const fs = require('fs');
        const parser = new me.DOMParser();
        let dom;
    ...
    var XMLHelpers = new function () {
        ...
        */
        me.parseXML = function (xmlString) {
            var myDocument;
            if (document.implementation && document.implementation.createDocument && config.DOMParser) {
                // All except IE, create a new DOMParser
                var parser = new config.DOMParser();
